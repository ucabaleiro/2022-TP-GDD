...
IF EXISTS (SELECT * FROM sys.views WHERE name = '[MARCO_AND_FRIENDS].[BI_MEJORES_TIEMPOS_ESCUDERIAS]' and type = 'V')
DROP VIEW [MARCO_AND_FRIENDS].[BI_MEJORES_TIEMPOS_ESCUDERIAS];
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = '[MARCO_AND_FRIENDS].[BI_TOP_3_CIRCUITOS_TIEMPO_BOXES]' and type = 'V')
DROP VIEW [MARCO_AND_FRIENDS].[BI_TOP_3_CIRCUITOS_TIEMPO_BOXES];
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = '[MARCO_AND_FRIENDS].[BI_TOP_3_CIRCUITOS_PELIGROSOS]' and type = 'V')
DROP VIEW [MARCO_AND_FRIENDS].[BI_TOP_3_CIRCUITOS_PELIGROSOS];
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = '[MARCO_AND_FRIENDS].[BI_PROMEDIO_INCIDENTES_ESCUDERIA_POR_ANIO_POR_SECTOR]' and type = 'V')
DROP VIEW [MARCO_AND_FRIENDS].[BI_PROMEDIO_INCIDENTES_ESCUDERIA_POR_ANIO_POR_SECTOR];
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = '[MARCO_AND_FRIENDS].[BI_CANTIDAD_PARADAS_POR_ESCUDERIA_POR_CIRCUITO]' and type = 'V')
...
CREATE TABLE [MARCO_AND_FRIENDS].[Incidente] (
    ...
    [codigo_sector] int NOT NULL,
    [codigo_carrera] int NOT NULL FOREIGN KEY REFERENCES [MARCO_AND_FRIENDS].[Carrera],
	[codigo_circuito] int NOT NULL FOREIGN KEY REFERENCES [MARCO_AND_FRIENDS].[Circuito] ([codigo_circuito]),
...
([codigo_circuito], [codigo_sector], [codigo_carrera],
...
GROUP BY M.CIRCUITO_CODIGO, M.CODIGO_SECTOR, M.INCIDENTE_BANDERA, M.CODIGO_CARRERA, M.INCIDENTE_TIPO, M.INCIDENTE_TIEMPO, M.INCIDENTE_NUMERO_VUELTA

...
CREATE TRIGGER controlFKProvClie ON Provincia AFTER DELETE
AS
BEGIN
    DECLARE cursorProvincias CURSOR FOR
    SELECT pcia_id FROM deleted
    ;
    DECLARE @provinciaID int;
    DECLARE @error BIT;
    OPEN cursorProvincias;

    FETCH NEXT FROM cursorProvincias INTO @provinciaID;
    WHILE @@FETCH_STATUS = 0
    BEGIN
        IF (EXISTS (SELECT * FROM Cliente WHERE pcia_id = @provinciaID))
        BEGIN
            PRINT 'No se puede eliminar la provincia ' + @provinciaID 
                + ' porque hay clientes que la tienen como FK';
            SET @error = 1;
        END
        FETCH NEXT FROM cursorProvincias INTO @provinciaID;
    END
    CLOSE cursorProvincias;
    DEALLOCATE cursorProvincias;
